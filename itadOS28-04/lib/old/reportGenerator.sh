...
reportGenerator() {
    ...
    #
    # If disks were not selected or disks not installed then notify this on the report.
    # Else have specs of the erasure per disk inserted into the report
    #
    if [ -z "$(ls -A "$TMP_REPORTS")" ]; then
        echo "Erasure cancelled or no storage drives installed." >> "$REPORT"
        echo "" >> "$REPORT"
        echo "" >> "$REPORT"
